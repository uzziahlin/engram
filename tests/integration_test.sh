...
# 工具集: 12 基础(含 query_stats、get_memory) + 1 collect_sources + 6 lifecycle + 4 reflection = 23
if [ "$TOOL_COUNT" = "23" ]; then
    assert_pass "1.2a 返回 23 个工具"
else
    assert_fail "1.2a 返回 23 个工具 (actual=$TOOL_COUNT)"
fi

# 验证所有工具名称（delete_xxx 已替换为 forget_memory/restore_memory 等生命周期工具）
EXPECTED_TOOLS=(search_memory get_memory related_files timeline recent_failures architectural_decisions query_stats create_episodic create_decision create_failure create_procedural ingest_commits collect_sources forget_memory restore_memory update_memory forget_batch list_archived consolidate_memories reflect list_suggestions confirm_suggestion reject_suggestion)
...
# ingest 产物是 milestone 记忆(content 以 "Milestone" 开头、带 source:ingest tag),
# 用必然存在的词做确定性断言 — 用 "feat" 依赖仓库近期提交内容,历史漂移会误报。
RESP=$(mcp_call '{"jsonrpc":"2.0","id":89,"method":"tools/call","params":{"name":"search_memory","arguments":{"project_id":"e2e-test","query":"milestone"}}}')
TEXT=$(extract_text "$RESP")
