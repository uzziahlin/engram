...
Two query shapes route differently — pick by intent:

- **Natural language** (whitespace-separated prose, e.g. `how to debug auth`):
  intent-routed — prioritizes memory types relevant to the detected intent
  (Debugging→failures+episodic, Architecture→decisions, …). Best for *concept*
  queries where you want noise reduced.
- **Identifier / exact lookup** (a single token with `-` or `_`, e.g. a tag
  `unique-tag-test-xyz`, filename `auth_utils`, symbol `parse_duration`):
  classified as General and searches **all** memory types for full recall.
  Pass it verbatim — don't split it into words.

Tokenization: FTS5 splits on `-`/`_`/punctuation, so `auth-utils` matches both
`auth` and `utils`. CJK is matched per-character.

Tags: search by the tag string directly (tags are indexed).

When results are thin: flip query shape (prose ↔ identifier), widen the
wording, or pivot via `related_files` (memories touching a file).
